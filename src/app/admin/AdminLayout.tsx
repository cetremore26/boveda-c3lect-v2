      ...
      <nav className="flex-1 px-3 py-4 flex flex-col gap-1">
        {NAV_ITEMS.map(({ to, label, icon: Icon, end }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded text-sm tracking-wide transition-colors border-l-2 ${
                isActive
  ...
  return (
    <AdminRoute>
      <div className="min-h-screen flex" style={{ background: '#0A0A0A' }}>
        {/* Sidebar desktop */}
        <aside
          className="hidden lg:flex flex-col w-56 shrink-0 border-r fixed top-0 bottom-0 left-0 z-30"
        ...
        {/* Sidebar mobile */}
        <aside
          className={`fixed top-0 bottom-0 left-0 z-50 w-56 lg:hidden transition-transform duration-200 ${
        ...
        <div className="flex-1 lg:ml-56 min-w-0">
          ...
          >
            ...
            <span
              className="text-base tracking-[0.22em]"
              style={{ fontFamily: 'var(--font-sans)', fontWeight: 300, color: '#C9A84C' }}
            >
              C3LECT
            </span>
